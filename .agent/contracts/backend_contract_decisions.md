...
### 여행방 생성 직후 투표와 스티커 에셋

- 여행방을 만들면 목록에 머물지 않고 지도(`/trips/:id/route?voteSetup=1`)로 바로 간다. 방장이고 세션이 없으면 배경이 블러된 투표 설정 모달을 한 번 띄운다.
  `지금은 나중에 할게요`로 닫으면 지도만 남고, 여행 카드의 `투표 시작`으로 언제든 다시 연다. 세션이 이미 있으면 이 쿼리는 무시한다.
- 스티커 에셋은 `frontend/public/vote-stickers/*.svg` 6종(하트 도장·별·깃발 핀·여권 스탬프·스마일·왁스 실링)이며 기본은 하트 도장이다.
  `voteStickerCatalog.ts`가 스타일과 경로를 관리하고 `VoteStickerMark.vue`가 한 장을 그린다. 덱은 붙인 수만큼(최대 8) 사진 왼쪽 위에 겹쳐 쌓고, 보드는 지급량만큼 자리를 두어 쓴 것만 선명하게 보인다.
- 에셋은 `src/assets`가 아니라 `public/`에 둔다. Vite가 4KB 미만 SVG를 data: URI로 인라인하면 `<use>`/`<img>` 참조가 환경에 따라 깨진다(지도 스티커 버그의 원인과 같다).
