...
if(p.endsWith('/saved-places'))body={items:Array.from({length:12},(_,i)=>({place:{provider:'KTO',externalPlaceId:String(i),name:['경복궁','함덕 해수욕장','북촌 한옥마을','서울숲'][i%4],address:'서울특별시 종로구',thumbnailUrl:photos[i%2]}}))};
...
for(const width of [1440,390]){await page.setViewportSize({width,height:1000});await page.goto('http://localhost:5173/mypage');await page.locator('.keepsake-note').first().waitFor();await page.getByRole('link',{name:'모두 보기 ›'}).first().click(); await page.locator('.saved-note-board').waitFor(); const search=await page.locator('.saved-board-search').boundingBox();const close=await page.locator('.story-overlay-close').boundingBox();if(search.y<close.y+close.height)throw Error('Search overlaps close');await page.getByRole('button',{name:'다음 페이지',exact:true}).click();if(await page.locator('.saved-note-board .mypage-place-card').count()!==4)throw Error('Pagination failed');await page.screenshot({path:__dirname+'/saved-board-modal-'+width+'.png'});await page.getByRole('button',{name:'닫기',exact:true}).click();await page.locator('.keepsake-board').scrollIntoViewIfNeeded();await page.screenshot({path:__dirname+'/mypage-board-'+width+'.png'});await page.locator('.taste-signature').scrollIntoViewIfNeeded();await page.screenshot({path:__dirname+'/mypage-taste-'+width+'.png'});if(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth))throw Error('Horizontal overflow');}
