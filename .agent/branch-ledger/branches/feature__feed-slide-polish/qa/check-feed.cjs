...
await page.waitForFunction(()=>!document.querySelector('.photo-prev-enter-active'));
const window=page.locator('.story-feed-window');await window.hover();await page.evaluate(()=>{window.feedFrames=[];let start=performance.now();function sample(){const w=document.querySelector('.story-feed-window');const el=document.querySelector('[data-story-id="2"]');window.feedFrames.push({t:performance.now()-start,scroll:w.scrollTop,y:el?.getBoundingClientRect().y,transform:el&&getComputedStyle(el).transform,gap:el&&document.querySelector('[data-story-id="1"]')?el.getBoundingClientRect().top-document.querySelector('[data-story-id="1"]').getBoundingClientRect().bottom:0});if(performance.now()-start<1400)requestAnimationFrame(sample)}requestAnimationFrame(sample)});await page.mouse.wheel(0,160);
await page.evaluate(async()=>{await new Promise(r=>setTimeout(r,100));const el=document.querySelector('.story-feed-window');for(let i=0;i<5;i++){el.dispatchEvent(new WheelEvent('wheel',{deltaY:60,bubbles:true,cancelable:true}));await new Promise(r=>setTimeout(r,240));}});
if(await page.locator('.story-post').getAttribute('data-story-id')!=='2')throw Error('Unexpected story after momentum: '+await page.locator('.story-post').getAttribute('data-story-id'));
...
if(Math.max(...frames.map(f=>f.gap||0))>1)throw Error('Blank gap between slides');
...
const box=await window.boundingBox();await page.mouse.move(box.x+box.width/2,box.y+150);await page.mouse.down();await page.mouse.move(box.x+box.width/2,box.y+50,{steps:10});const preview=page.locator('.feed-drag-preview');await preview.waitFor();
const coverage=await page.evaluate(()=>{const a=document.querySelector('.story-post').getBoundingClientRect();const b=document.querySelector('.feed-drag-preview').getBoundingClientRect();return b.top-a.bottom});if(Math.abs(coverage)>1)throw Error('Blank drag preview gap');
await page.mouse.up();
