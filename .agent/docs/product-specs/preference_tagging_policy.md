...
중요한 점은 이 값이 "좋아하는 태그"를 의미하지 않는다는 것이다. 평균보다 훨씬 싫어하는 태그도 취향을 잘 가르는 태그다. 좋아함/싫어함의 방향은 사용자별 `user_preference_tag_weights.preference_score`가 표현한다.
...
```text
place_tag_raw_evidence = confidence * weight
```

한 장소가 사용자 취향에 주는 근거 합은 1이다.
...
`preference_score`는 `0..1` 범위이며 `0.5`가 중립이다. 태그 전체 평균의 사전 강도와 호불호에 따른 개인 반응 반영 속도는 별도 계산 policy version으로 관리하고, 검증 전 상수를 고정하지 않는다.

## 추천 점수

후보 장소 추천 점수는 사용자 선호도와 장소 확정 태그 비율의 가중평균이다.
...
장소별 `normalized_place_tag_evidence` 합은 1이므로 멤버별 장소 점수는 `0..1` 범위를 유지한다.

다른 멤버의 긍정/부정 근거, preference score, 세부 태그 가중치는 API와 UI에 노출하지 않는다. 추천 카드에는 matched member avatar 수준만 노출한다.
