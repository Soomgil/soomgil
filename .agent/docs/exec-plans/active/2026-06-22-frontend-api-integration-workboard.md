...
| `FEI-141` | 커뮤니티 홈·스토리 조회 API 연동 | `DONE` | 없음 | frontend PR #5, `8a6670b` | 목록·상세·댓글 component test 및 로컬 API smoke 통과 |
| `FEI-142` | 스토리 작성·사진 업로드 API 연동 | `DONE` | 없음 | frontend PR #5, `8a6670b` | 여행·기록 사진·등록 DTO test 및 로컬 API smoke 통과 |
| `FEI-143` | 스와이프 추천·반응 API 연동 | `DONE` | 없음 | frontend PR #5, `8a6670b` | feed/reaction/retry test 및 로컬 LIKE 저장 확인 |
| `FEI-150` | 미사용 `UserProfileModal` 제거 또는 실제 API 전환 | `DONE` | 사용처 재확인 | frontend PR #5, `8a6670b` | route·template·dynamic import 미사용 확인 후 component 제거 |
| `FEI-160` | AI API client의 hard-coded session/history 제거 | `DONE` | 실제 AI DTO 확인 | frontend PR #5, `8a6670b` | session/messages/chat DTO, empty/failure/retry test 및 GMS history smoke 통과 |
| `FEI-170` | notification 목록 client의 hard-coded 빈 응답 제거 | `DONE` | 없음 | frontend PR #5, `8a6670b` | empty/pagination/read failure test 및 로컬 알림 조회 통과 |
| `FEI-171` | `LandingPage` 및 인증 화면 연동 점검 | `DONE` | 없음 | frontend PR #5, `8a6670b` | 인증·reset token link, OAuth 실패, 완료 안내 test 통과 |
| `FEI-180` | production mock/stub 재유입 방지 검사 추가 | `DONE` | `FEI-100`~`171` | frontend PR #5, `8a6670b` | `frontend:api-integrity` 및 harness 연결 |
...
- [x] `CommunityPage`, `StoriesPage`, `FeedPage`의 loading·empty·error·retry와 목록/상세/댓글 조회를 검증했다.
- [x] `StoryWritePage`의 더미 초기 글을 제거하고 `tripApi`, record photo, media upload, story create DTO를 연결했다.
- [x] 피드 빈 상태의 작성 경로를 `/community/story-write`로 바로잡았다.
- [x] `SwipePage`의 feed, LIKE/NOPE/SUPER_LIKE 저장, 실패 재시도를 test와 로컬 backend에서 검증했다.
...
- [x] template, dynamic component와 route를 포함해 실제 사용처를 재확인한다.
- [x] 미사용 component와 scoped style/type을 함께 제거한다.
- [x] 사용자 프로필 UI는 실제 API를 사용하는 `UserProfilePage`로 단일화한다.
- [x] 같은 사용자 프로필 UI를 두 군데에서 별도로 유지하지 않는다.
...
- [x] backend DTO에 맞춰 session, messages와 chat response type을 수정한다.
- [x] hard-coded session/history를 실제 GET으로 바꾼다.
- [x] route draft를 itinerary tool call 결과 동기화로 대체한다.
- [x] 사용되지 않는 잘못된 route draft client/type을 제거한다.
- [x] 정상, 빈 history, AI 실패와 재시도 테스트를 작성한다.
...
- [x] `GET /api/v1/notifications`에 pagination parameter를 연결한다.
- [x] backend page DTO와 frontend type을 맞춘다.
- [x] 읽음 처리 후 목록과 unread count를 갱신한다.
- [x] 빈 목록, pagination, 읽음 실패 테스트를 작성한다.

### FEI-171 — LandingPage와 인증 화면

작업:

- [x] `LandingPage`, 로그인, 가입, 이메일 인증과 비밀번호 재설정 흐름을 실제 auth API 기준으로 재점검한다.
- [x] 인증 만료·OAuth callback·서버 오류 상태의 사용자 안내를 검증한다.
- [x] production mock 또는 임시 성공 fallback이 남아 있는지 확인한다.

### FEI-180 — mock/stub 재유입 방지

작업:

- [x] test와 story fixture를 제외한 production 코드의 `@/mocks` import를 검사한다.
- [x] API client의 async method가 HTTP/fetch 경계 없이 hard-coded 응답을 반환하는지 AST로 검사한다.
- [x] `community.api.ts#toggleLike`의 동일 client method 위임을 파일·method·이유와 함께 허용 예외로 명시한다.
- [x] 검사를 `.agent` `harness:check`에 연결한다.
...
| 2026-06-23 | `FEI-141`~`FEI-180` | `TODO` → `VERIFY` | `feature/frontend-api-integration-fei-141-180` | 커뮤니티·작성·스와이프·AI·알림·인증 연동 보완과 mock/stub 자동 검사 완료; develop 반영 대기 |
| 2026-06-23 | `FEI-141`~`FEI-180` | `VERIFY` → `DONE` | frontend PR #5, `8a6670b` | 원격 `develop` merge, 기능 브랜치 삭제 및 submodule pointer 갱신 |
...
| 2026-06-23 | `FEI-141`~`FEI-180` | `npm.cmd run test:run`, `npm.cmd run build` | frontend 36 files/182 tests 및 production build 통과 |
| 2026-06-23 | `FEI-180` | `npm.cmd --prefix .agent run frontend:api-integrity` | production 109개, API client 16개 파일 통과; 허용 예외 1개 출력 |
| 2026-06-23 | 로컬 통합 | in-app browser + backend `8080` | 커뮤니티 목록/상세/댓글, 여행·기록 사진, 스와이프 LIKE, 알림 page, AI session/history 통과; console error 0 |
| 2026-06-23 | root harness | `npm.cmd --prefix .agent run harness:check` | API integrity, branch ledger, frontend dist, SPA 20 routes 통과 |
