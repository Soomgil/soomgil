...
| `FEI-100` | media 기록 목록 client의 mock 제거 | `DONE` | 없음 | frontend `feature/record-api-integration`, backend `feature/record-photo-summaries` | 실제 page·summary·signed read URL 계약 및 테스트 |
| `FEI-110` | `RecordPage` 여행·기록·사진 실제 API 연동 | `DONE` | `FEI-100` | frontend `feature/record-api-integration`, backend `feature/record-photo-summaries` | 무한 스크롤, 업로드 idempotency와 media 정리 정책 검증 |
...
- `GET /api/v1/records/photos`
- `GET /api/v1/trips/{tripId}/records/photos`
- `POST /api/v1/records/photo-summaries`
- `GET /api/v1/records/photos/{mediaFileId}/read-url`

작업:

- [x] 실제 page response 타입을 backend DTO와 맞춘다.
- [x] `getRecords()`를 실제 GET 호출로 바꾼다.
- [x] 전역/여행별 사진, summary와 signed read URL client를 추가한다.
- [x] mock import를 제거한다.
...
- [x] 여행 목록을 `tripApi`에서 가져온다.
- [x] route의 `tripId`가 있으면 해당 여행을 우선 선택한다.
- [x] 선택한 여행의 photo를 실제 API와 무한 스크롤로 조회한다.
- [x] 전체/여행별 사진 개수와 대표 사진을 page metadata와 batch summary로 표시한다.
- [x] loading, empty, error, retry와 접근 권한 실패를 구분한다.
- [x] 여행 전환 시 오래된 응답이 현재 화면을 덮지 않도록 요청 경합을 처리한다.
- [x] 사진 업로드와 기록 생성을 연결하고 불확실한 실패는 같은 `Idempotency-Key`로 재시도한다.
- [x] upload intent 24시간 정리와 soft-delete media 7일 purge를 backend에서 수행한다.
...
| 2026-06-22 | `FEI-100`, `FEI-110` | frontend test/build, backend 전체 test, OpenAPI parse와 harness check | frontend 28 files/135 tests, backend 181 suites/589 tests, production build와 실제 MinIO compose smoke 통과 |
