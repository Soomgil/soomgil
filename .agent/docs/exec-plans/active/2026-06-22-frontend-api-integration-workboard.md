...
| `FEI-100` | media 기록 목록 client의 mock 제거 | `VERIFY` | 없음 | frontend `49326b1`, backend `c5d1000` | 실제 page·photo summary·signed read URL 계약 검증 |
| `FEI-110` | `RecordPage` 여행·기록·사진 실제 API 연동 | `VERIFY` | `FEI-100` | frontend `49326b1`, backend `c5d1000` | frontend 135 tests/build, backend 589 tests 통과 |
| `FEI-120` | `HomePage` 여행·커뮤니티 실제 API 연동 | `IN_PROGRESS` | 없음 | frontend `feature/mypage-api-integration` 기반 stash 보존 | 구현 후 코드 검토·정식 브랜치 커밋 필요 |
| `FEI-121` | Home 주간 인기 장소 TOP 3 연동 | `TODO` | backend 인기 장소 API | - | 전체 사용자 기준 주간 TOP 3로 제품 결정 완료 |
| `FEI-130` | `MyPage` 사용자·팔로우·저장 장소 mock 제거 | `VERIFY` | `FEI-010` | frontend `33c138a`, `2aeeb66`; backend `773aa56` | frontend build 통과, develop 반영 대기 |
| `FEI-131` | `MyTripsPage` 내 여행 목록 실제 API 연동 확인 및 보완 | `VERIFY` | 없음 | frontend `968685f`, root `52a2e30` | frontend 113 tests/build, 하네스 통과 |
| `FEI-132` | `SettingsPage` 사용자·알림 설정 API 연동 | `TODO` | 없음 | - | - |
| `FEI-133` | `TripInviteAcceptPage` 초대 수락 API 연동 | `TODO` | 없음 | - | - |
| `FEI-140` | `RoutePage`의 `mockPlaces` fallback 제거 | `VERIFY` | 없음 | frontend `9e61dde`, root `8dbb0d8` | frontend 테스트 통과, develop 반영 대기 |
...
- 여행과 커뮤니티 카드의 실제 API 전환 변경은 stash에 보존되어 있다.
- 정식 브랜치 커밋 전 코드 검토와 전체 회귀 검증이 남아 있다.
...
- [x] 로그인하지 않은 사용자와 인증 만료 상태를 처리한다.
- [x] 부분 실패 시 성공한 카드까지 숨기지 않는다.
- [x] 카드 link가 실제 trip/story ID를 사용하도록 검증한다.
- [ ] 보존된 변경을 정식 작업 브랜치로 복원해 코드 검토·전체 테스트·커밋한다.
...
- 현재 backend에는 이 집계 의미를 보장하는 endpoint가 없어 구현 대기 상태다.
...
- **FEI-132 (`SettingsPage`)**: 사용자 프로필과 알림 설정을 실제 조회·변경 API에 연결한다.
- **FEI-133 (`TripInviteAcceptPage`)**: 초대 코드 검증·수락 후 실제 여행방으로 이동하는 흐름을 검증한다.
